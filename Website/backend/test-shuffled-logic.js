...
function testRotation() {
    ...
    const QUESTIONS_PER_CYCLE = 240;
    const DAYS_PER_CYCLE = 24;

    // Test Cycle 1 (Days 1 to 21)
    console.log("Cycle 1 (Days 1-21):");
    ...
    // Test Cycle 2 (Days 25 to 48)
    console.log("\nCycle 2 (Days 25-48):");
    const cycle2Ids = new Set();
    const day25Ids = logic.getQuestionIds(25); // Start of Cycle 2
    const day1Ids = logic.getQuestionIds(1);

    console.log(`Day 1:  [${day1Ids.join(', ')}]`);
    console.log(`Day 25: [${day25Ids.join(', ')}]`);

    let overlap = day1Ids.filter(id => day25Ids.includes(id));
    if (overlap.length === 10 && day1Ids[0] === day25Ids[0]) {
    ...
    // Verify all 240 are present in cycle 2
    for (let day = 25; day <= 48; day++) {
        const ids = logic.getQuestionIds(day);
